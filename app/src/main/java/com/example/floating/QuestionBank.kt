...
object QuestionBank {
    ...
    private fun generateAdvancedMathQuestion(typeIdx: Int = -1): Pair<Question, String> {
        ...
        18 -> { val start_h = Random.nextInt(7, 10); val mins = listOf(15, 30, 45, 60).random(); val end_h = start_h + mins / 60; val end_m = mins % 60; val end_m_str = if (end_m == 0) "00" else "$end_m"; createQuestion("小欣 $start_h:00 开始写作业，写了 $mins 分钟，( ) 写完。", "$end_h:$end_m_str", listOf("$start_h:$end_m_str", "${end_h + 1}:00", "$end_h:${(end_m + 10) % 60}")) }
        19 -> { val face = listOf("北", "南", "东", "西").random(); val back = mapOf("北" to "南", "南" to "北", "东" to "西", "西" to "东")[face]!!; createQuestion("小明面向$face，他的后面是什么方向？", back, listOf("东", "西", "南", "北").filter { it != back }) }
        20 -> { val yuan = Random.nextInt(1, 10); val jiao = listOf(5, 10, 50).random(); val total_jiao = yuan * 10 + jiao; createQuestion("$yuan 元 $jiao 角 = ( ) 角", "$total_jiao", listOf("${yuan * 10}", "${total_jiao + 5}", "${total_jiao - 5}")) }
        21 -> { val n = Random.nextInt(2, 4); val total = n * (n - 1); val digitList = (1..n).joinToString("、"); createQuestion("用 $digitList 这$n个数字可以组成 ( ) 个没有重复数字的两位数。", "$total", listOf("${total - 1}", "${total + 1}", "${total + 2}")) }
        22 -> { val a = Random.nextInt(2, 9); val b = Random.nextInt(1, 9); if (a + b < 10) { createQuestion("${a} + $b = ( )", "${a + b}", listOf("${a + b + 1}", "${a + b - 1}", "${a * b}")) } else { createQuestion("${a}0 + $b = ( )", "${a * 10 + b}", listOf("${a * 10 + b - 1}", "${a * 10 + b + 1}", "${(a + 1) * 10}")) } }
        ...
        24 -> { val shapes = listOf(Triple("正方体", "6"), Triple("三棱柱", "5"), Triple("圆锥", "2"), Triple("球", "1")); val s = shapes.random(); createQuestion("${s.first}有几个面？", s.second, listOf("2", "3", "4", "5", "6", "8").filter { it != s.second }.take(3)) }
